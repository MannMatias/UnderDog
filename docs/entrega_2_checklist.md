...
- [x] **Outputs guardados** en el `.ipynb` (tablas y 5 gráficos).
- [x] **Shape**: 17.959 filas × 31 columnas (Parte 1).
- [x] **Tipos**: tabla de dtype + rol + origen de las 31 columnas (Parte 1).
...
- [x] **4 hipótesis** con las 6 casillas, máximo 2 movimientos, gráfico, resultado cuantitativo, zona y decisión (Parte 2).
  - al menos una de dominio: H1 y H2;
  - al menos una que decide sobre columnas: H3 y H4.
- [x] **Al menos una refutada/inconclusa**: H3 🔴 refutada, H2 🟡 inconclusa.
- [x] **Tabla de columnas candidatas**: Parte 3 del notebook + `docs/column_candidates.md` / `.csv`. Tiene las 82 columnas de la
  Entrega 1 + las nuevas, con zona, decisión y si existiría al predecir.
...
- [x] 84 tests en el contenedor de Airflow (`pytest tests`, Airflow 3.3.1 + pandas 2.3); 63 en local sin Airflow (los del DAG se saltean).
